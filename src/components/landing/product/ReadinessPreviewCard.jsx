...
const competencies = [['Strategic Thinking', 82], ['Decision Quality', 76], ['Communication', 88], ['Leadership Presence', 71], ['Enterprise Judgment', 79]];
...
export default function ReadinessPreviewCard() {
  return <article className="group rounded-2xl border border-white/10 bg-white/[0.025] p-5 md:p-6">
    <div className="mb-4 flex items-center justify-between text-[10px] uppercase tracking-wider text-white/40"><span>Executive Readiness Report™</span><span className="text-emerald-400">Illustrative Demo</span></div>
    <div className="rounded-xl border border-white/10 bg-black/20 p-4">
      <div className="flex items-center gap-4"><div className="flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-full border-4 border-amber-400/70"><strong className="text-2xl text-white">79</strong><span className="text-[9px] text-white/40">READINESS</span></div><div><p className="text-xs text-white/40">Readiness trajectory</p><p className="mt-1 flex items-center gap-1 text-sm font-semibold text-emerald-400"><TrendingUp size={14}/>Improving</p><p className="mt-2 text-[11px] text-white/50">Strength: strategic judgment<br/>Gap: executive presence</p></div></div>
    ...
    <Link to="/executive-readiness" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-accent-orange">Explore Readiness <ArrowRight size={14}/></Link>
  </article>;
}