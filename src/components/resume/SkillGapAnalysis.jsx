...
export default function SkillGapAnalysis({ data, targetRole }) {
  ...
  const leadSkills = data.leadership_skills || [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
          ...
          {techSkills.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">{techSkills.map((s, i) => <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-indigo-500/10 text-indigo-400">{s}</span>)}</div>
        ...
        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
          ...
          {leadSkills.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">{leadSkills.map((s, i) => <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-violet-500/10 text-violet-400">{s}</span>)}</div>
          ) : <p className="text-white/30 text-sm">None identified</p>}
        </div>
      </div>
      ...
      {certs.length > 0 && (
        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
          ...
          <div className="flex flex-wrap gap-1.5">{certs.map((c, i) => <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-emerald-500/10 text-emerald-400">{c}</span>)}</div>
        </div>
      )}
    </div>
  );
