...
import { FileText, Loader2, ChevronRight, Plus } from "lucide-react";
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      <div className="lg:col-span-1 space-y-2">
        ...
        {Object.entries(grouped).map(([name, versions]) => {
          ...
          return (
            <button key={name} onClick={() => { setSelectedId(latest.id); setShowCompare(false); }} className={`w-full text-left p-3 rounded-xl border transition-colors ${isActive ? "bg-indigo-500/10 border-indigo-500/30" : "bg-white/[0.02] border-white/5 hover:bg-white/5"}`}>
