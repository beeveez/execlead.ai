...
export default function Onboarding() {
  ...
  return (<OnboardingShell step={type === "enterprise_assigned" ? 1 : step}>{error && <p role="alert" className="mb-4 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">{error}</p>}{type === "enterprise_assigned" ? <EnterpriseLaunch user={user} onLaunch={launchEnterprise} /> : step === 1 ? <WelcomeOrientation onContinue={() => go(2)} onSkip={later} /> : step === 2 ? <QuickReadinessBaseline baseline={baseline} onChange={setBaseline} onComplete={complete} onLater={later} saving={saving} /> : <OnboardingComplete result={result || { readinessLevel: user?.readiness_level || "Calibrated", xp: user?.journey_points || 0 }} onContinue={() => { window.location.href = "/dashboard"; }} />}</OnboardingShell>);
}