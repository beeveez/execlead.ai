...
/**
 * Agent Orchestration Service™ — Phase 5 (controlled proof-of-execution)
 * ============================================================
 * The ONLY live Workforce execution capability in Phase 5:
 ...
 * - AgentExecution records are created by this service role ONLY, before the
...
const PHASE5_AGENT_ID = 'exec_concierge';
const PHASE5_TOOL_ID = 'read_own_readiness_assessment';
const PHASE5_TARGET_TYPE = 'ENTITY';
const PHASE5_TARGET_NAME = 'ReadinessAssessment';
const PHASE5_OPERATION = 'READ';
const PROVENANCE_SOURCE = 'agent_orchestration_service';
const PROVENANCE_META = {
  provenance: 'server_orchestration',
  issued_by: 'agentOrchestrationService',
  phase: 5,
};
const CONFIG_ID = 'agent_orchestration_global';
const RISK_LEVELS = { low: 0, medium: 1, high: 2, critical: 3 };
const IDEMPOTENCY_TTL_MS = 60000; // replay window for the read-only capability
const PLATFORM_ROLES = ['super_admin', 'platform_admin', 'founder_root_admin'];

function correlationIdFor(userId) {
  return `wf:${userId}:${PHASE5_AGENT_ID}:${PHASE5_TOOL_ID}`;
}

/** Record a BLOCKED execution attempt (audit stays truthful; nothing executes). */
async function recordBlocked(svc, user, errorCode, errorMessage, extraMeta = {}) {
  ...
  const execution = await svc.entities.AgentExecution.create({
    ...
    requested_at: now,
    correlation_id: correlationIdFor(user.id),
    error_code: errorCode,
    error_message: errorMessage,
    source: PROVENANCE_SOURCE,
    metadata: { ...PROVENANCE_META, ...extraMeta },
  });
  return execution;
}

function blockedResponse(errorCode, message, execution) {
...
async function executeReadinessRead(svc, user, body) {
  ...
  if (body.agent_id !== undefined && body.agent_id !== PHASE5_AGENT_ID) {
    const execution = await recordBlocked(svc, user, 'AGENT_IDENTITY_MISMATCH',
      ...
      { claimed_agent_id: String(body.agent_id) });
  ...
  if (body.tool_id !== undefined && body.tool_id !== PHASE5_TOOL_ID) {
    const execution = await recordBlocked(svc, user, 'TOOL_IDENTITY_MISMATCH',
      ...
      { claimed_tool_id: String(body.tool_id) });
  ...
  if (!cfg) {
    const execution = await recordBlocked(svc, user, 'GLOBAL_EXECUTION_STOP',
      'Orchestration configuration missing — orchestrator fails closed.');
  ...
  if (cfg.stop_new_executions === true) {
    const execution = await recordBlocked(svc, user, 'GLOBAL_EXECUTION_STOP',
      'Global Workforce execution stop is active.', { global_stop: true });
    return blockedResponse('GLOBAL_EXECUTION_STOP',
      'Workforce execution is globally stopped.', execution);
  }
  ...
  if (!allowed) {
    const execution = await recordBlocked(svc, user, 'AGENT_NOT_AUTHORIZED_FOR_TOOL',
      'Agent is not explicitly listed on the tool allow-list.');
  ...
  if (tool.target_type !== PHASE5_TARGET_TYPE || tool.target_name !== PHASE5_TARGET_NAME
    ...
    const execution = await recordBlocked(svc, user, 'TOOL_TARGET_MISMATCH',
      'Tool target/operation does not match the Phase 5 registered capability.');
  ...
  if (tool.execution_scope !== 'user_scoped' || tool.permission_scope !== 'self_records'
    ...
    const execution = await recordBlocked(svc, user, 'SCOPE_MISMATCH',
      'Tool or agent scope is not user_scoped/self_records.');
  ...
  if (toolRisk > maxRisk) {
    const execution = await recordBlocked(svc, user, 'RISK_LEVEL_EXCEEDED',
      ...
      { tool_risk_level: tool.risk_level, max_risk_level: cfg.max_risk_level });
  ...
  if (tool.human_approval_required === true) {
    ...
    const execution = await svc.entities.AgentExecution.create({
      ...
      correlation_id: correlationId,
      source: PROVENANCE_SOURCE,
      metadata: { ...PROVENANCE_META, approval_id: approvalId },
    });
  ...
  const execution = await svc.entities.AgentExecution.create({
    ...
    source: PROVENANCE_SOURCE,
    metadata: { ...PROVENANCE_META, idempotency_ttl_ms: IDEMPOTENCY_TTL_MS },
  });
...
async function getExecution(svc, user, body) {
 ...
 */
async function decideApproval(svc, user, body) {
