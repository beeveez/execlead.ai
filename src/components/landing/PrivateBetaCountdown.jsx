  ...
  return <section className={`private-beta-countdown ${phase.compact ? 'launch-phase-final' : ''} relative isolate overflow-hidden px-4 py-16 sm:px-6 sm:py-24 lg:py-28`} aria-labelledby="private-beta-countdown-title">
    ...
    <div className="relative z-10 mx-auto max-w-6xl text-center">
      ...
      {phase.live ? <div className="mt-8" aria-live="polite"><p className="text-base text-launch-muted sm:text-lg">{phase.message}</p><Link to={primaryPath} className={`${actionClass} mt-8 bg-launch-accent text-launch-background`}>Begin Your Executive Journey</Link></div> : <>
        ...
        <div className={phase.compact ? 'sr-only' : 'mt-10'}><p className="font-display text-base font-semibold uppercase tracking-[0.3em] text-launch-foreground sm:text-lg">{PRIVATE_BETA_MILESTONE}</p><time dateTime={PRIVATE_BETA_LAUNCH_AT} className="mt-2 block text-sm font-medium tracking-[0.16em] text-launch-foreground/80 sm:text-base">{PRIVATE_BETA_LAUNCH_LABEL}</time>{phase.message && <p className="mx-auto mt-5 max-w-xl text-sm leading-6 text-launch-muted sm:text-base">{phase.message}</p>}<p className="mt-4 text-sm font-medium text-launch-foreground">One Leadership Journey. One AI Platform.</p></div>
