...
import { buildComparisonMatrix, FEATURE_ROWS, EXECLEAD_AI_MATRIX } from "@/lib/competitiveIntelligence";
...
export default function IntelReports({ competitors }) {
  ...
  const exportMatrixCsv = () => {
    ...
    const rows = matrix.map((r) => [r.feature, ...cols.map((c) => r[c])].map((v) => `"${String(v)}"`).join(","));
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4"><Download size={16} className="text-indigo-400" /><h2 className="text-lg font-semibold">Reports & Export</h2></div>
