...
export default function Skills() {
  ...
  const handleAnalyzeWithAI = async () => {
    setAnalyzing(true);
    try {
      const response = await base44.functions.invoke('skillsIntelligence', { action: 'importSkills' });
      const result = response.data || response;
      await loadSkills();
      toast({
        title: "AI Skill Import Complete",
        description: `✓ ${result.created} new skills added · ✓ ${result.updated} existing skills updated — Total: ${result.total}`,
      });
    } catch (err) {
      toast({ title: "Analysis Failed", description: err.response?.data?.error || err.message || "Could not analyze skills.", variant: "destructive" });
    } finally {
      setAnalyzing(false);
    }
  };
